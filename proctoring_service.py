...
class ProctoringSession:
    ...
    face_mesh_instance: any = None  # Session-specific MediaPipe instance
    
    def __post_init__(self):
        if self.violations is None:
            self.violations = []
        self.face_mesh_instance = mp.solutions.face_mesh.FaceMesh(
            max_num_faces=10,
...
class AdvancedFaceAnalyzer:
    ...
    def set_reference_face(self, image, face_mesh_instance=None):
        """Set reference face encoding from captured image - with session-specific face_mesh"""
        try:
            ...
            # Use session-specific face_mesh or fallback to shared instance
            mesh = face_mesh_instance if face_mesh_instance else self.face_mesh
    ...
    def detect_faces_and_analyze(self, input_image, face_mesh_instance=None):
        """
        ORIGINAL WORKING LOGIC - with session-specific face_mesh support
        """
        try:
            # Convert BGR to RGB
...
class ProctoringService:
    ...
    async def set_reference_face(self, session_id: str, image_data: str) -> Dict:
        """Set reference face from captured image - using session-specific face_mesh"""
        try:
            ...
            # Use session-specific face_mesh for reference face
            success = self.face_analyzer.set_reference_face(reference_image, session.face_mesh_instance)
    ...
    def _analyze_frame_fixed_reset(self, session: ProctoringSession, frame_img: np.ndarray) -> Dict:
        ...
        # Analyze frame with session-specific face_mesh
        analysis = self.face_analyzer.detect_faces_and_analyze(frame_img, session.face_mesh_instance)
